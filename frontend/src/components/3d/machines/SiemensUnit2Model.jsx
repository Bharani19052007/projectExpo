  ...
  return (
    <group>
      ...
      <group ref={groupRef}>
        ...
        <group userData={{ compId: 'conveyor-belt' }} position={[-2.5, 0.2, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'conveyor-belt')); }}>
          ...
          {[-1.3, 1.3].map((x, i) => (
            <mesh key={i} position={[x, 0, 0]} rotation={[Math.PI / 2, 0, 0]} userData={{ defaultColor: '#475569' }}>
        ...
        <group ref={armRef} userData={{ compId: 'packaging-arm' }} position={[-1.2, 0.8, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'packaging-arm')); }}>
          {/* Vertical Post */}
          ...
          <mesh position={[0.4, 0.4, 0]} userData={{ defaultColor: '#00507a' }}>
        ...
        <group userData={{ compId: 'carton-loader' }} position={[0, 0.4, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'carton-loader')); }}>
        ...
        <group userData={{ compId: 'servo-axis' }} position={[1.2, 0, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'servo-axis')); }}>
        ...
        <group userData={{ compId: 'pneumatic-cylinder' }} position={[2.0, 0.4, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'pneumatic-cylinder')); }}>
        ...
        <group userData={{ compId: 'vision-camera' }} position={[0, 1.6, 0.8]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'vision-camera')); }}>
        ...
        {/* 7. Barcode Scanner */}
        <group userData={{ compId: 'barcode-scanner' }} position={[-1.2, 1.4, 0.6]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'barcode-scanner')); }}>
          <mesh userData={{ defaultColor: '#a855f7' }}>
            <boxGeometry args={[0.25, 0.35, 0.25]} />
            <meshStandardMaterial color="#a855f7" emissive="#7e22ce" emissiveIntensity={0.3} />
          </mesh>
        </group>

        {/* 8. Heat Sealing Unit */}
        <group userData={{ compId: 'sealing-unit' }} position={[3.0, 0.4, 0]} onClick={(e) => { e.stopPropagation(); setSelectedComponent(components.find(c => c.id === 'sealing-unit')); }}>
          <mesh userData={{ defaultColor: '#dc2626' }}>
            <boxGeometry args={[0.8, 0.8, 0.8]} />
            <meshStandardMaterial color="#dc2626" roughness={0.3} metalness={0.7} />
          </mesh>
          {/* Glowing Ceramic Heat Bar */}
